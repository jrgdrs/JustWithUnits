...
Following the approach "Just' with units", the applicability of Frank E. Blokland's research results on “Patterning in Movable Latin Type” is examined in practical use. Narrow column widths in newspaper and magazine typography often form rivers of white space and create an uneven appearance, which limits legibility. An attempt will be made to compensate for these limitations with historically lost solutions.

Johannes Gutenberg and Francesco Griffo already used different widths of the same letters to achieve a balanced line pattern. This traditional process has been lost in the course of technical development. In this work, the functions are revived with the help of state-of-the-art technologies such as Opentype fonts and the quality is evaluated using a series of tests. The important foundation here is the consideration of patterning, with which the regularity required to produce a smooth block surface of paragraphs already begins with the individual letters. The restriction to the underlying patterns in the rhythm of the typeface is intended to maintain the ordered structure and avoid distractions in the reader's perception.
...
During the EcTd class, we used the LetterModeler to develop a Renaissance Antiqua, to which we added further characters based on the word “Imperial” and then came to the point of combining letters with different character widths into classes of the same width. In this task, all characters are first brought to a valid width that corresponds to a level of the cadences derived from the pattern of the respective font. The next step is to combine the different widths into as few classes as possible and, if necessary, to adapt the shapes of the characters accordingly. When working on this task, I found that the character widths could not always be clearly defined based on my internalized ideas of proportions. This meant that several variants could be valid in the same character set without disturbing the frequency of the cadence. So I came up with the idea of preserving the variants and providing them in a variable font. I remembered prototypical procedures with multiple master fonts from thirty years ago as a possible use for the width variants and discovered a potential benefit in improving justification. After initial investigations, the topic fascinated me and I decided to present this task for my personal project.

# Preanalysis

For an anticipated experiment, I first added changing widths for the most common characters in the German language (e, n, i, r, s, t, a, d, h) to my version of the exercise using the open type feature CALT (character alternative) and used this font to create a sample set, which I presented to my fellow students. They confirmed that the different widths were not immediately or easily recognizable.
...
*Test typesetting to check the conspicuousness of deviating character widths*

# Kick off

For the briefing, I created a presentation deck entitled “Unit based width variants of glyphs for balanced justification” that summarizes my preliminary analyses.
...
One suggestion I took away from the presentation was to also consider the optical edge compensation.

# Setting up a test environment

To ensure that the tests required for the evaluation could be provided as easily and reproducibly as possible, I looked for existing methods that already have functions for producing justified text.
...
*User interface of the solution prepared by Simon Cozens.*
...
*User interface of the solution prepared by Bram Stein.*
...
*User interface of the solution used here.*
...
*Comparison of character scaling methods*
...
*The provided axes of a variable font can be checked with tools such as Axis-Praxis, here the three axes used in the test font for with Weight, Counter Width and Width axis.*
...
*Traditional manual typesetting: As soon as no more characters fit into the composition stick, the necessary space between the lines is filled with spaces of different widths.*
...
- In this calculation, the width of the separator and the punctuation characters defined as overshooting is reduced by the specified factor (optical margin). The following characters were defined for this: overshootChars = [“-”, “.”, “:”, “,”, “;”]. For reasons of efficiency, the adjustment was only taken into account at the right-hand edge of the sentence during implementation.
   ...
   *It is worth mentioning that the hyphen had to be replaced by the line separator (uni2010) and the character had to be added to the font, as modern browser versions use the line separator and treat this character differently for accessibility reasons, e.g. the character should not be read out in audio output (topic: dehyphenation).*
...
![Bildschirmfoto 2025-05-02 um 21.40.32.png](https://res.craft.do/user/full/ce92c3bc-bf1f-2f0e-e611-560f02f61cc0/doc/37a2d212-9359-45ac-8a9b-ebdf231b405b/e9f6ad2c-ef7d-60cb-af9c-bc81e22e88e9/VBblOnD4vrzO6mcjYa1WjsdTgA9KT5AwuBEoyaUgZ4Qz/Bildschirmfoto%202025-05-02%20um%2021.40.32.png)

*Comparison of the set character widths with regard to the conspicuousness of the differences. (Examples 120%). To decide which characters should be used and in which order, the conspicuousness of the changes was ranked using the examples.*
...
![Bildschirmfoto 2025-05-02 um 20.52.46.png](https://res.craft.do/user/full/ce92c3bc-bf1f-2f0e-e611-560f02f61cc0/doc/37a2d212-9359-45ac-8a9b-ebdf231b405b/bd44fd0d-6824-530b-ef3e-447adc535afa/Ko1xtC0zdb9ExdZmnsZWHQkSrR0TJDTjtPqEunDxFO8z/Bildschirmfoto%202025-05-02%20um%2020.52.46.png)

*By widening the characters, here /e, /c, /s, /a, /i, /f, /r, /t, the whitespace is reduced to a minimum.*
...
![Bildschirmfoto 2025-05-03 um 09.02.40.png](https://res.craft.do/user/full/ce92c3bc-bf1f-2f0e-e611-560f02f61cc0/doc/37a2d212-9359-45ac-8a9b-ebdf231b405b/9c5a8b30-2dd2-63dc-cdf3-1105642bba3e/2y8TlVEz4eRIDyaHgr3KhoKGcmCixby0yCrJBCvZm58z/Bildschirmfoto%202025-05-03%20um%2009.02.40.png)

*Overlay without scaling (black), with the same character widths (light grey) and filled with wider character variants (white) using the unitized method. As a result, you can see that the middle letters of the words remain in position and the spaces are reduced by the widened characters.*

# Opentype Font

When extending the font, I started from the Imperial Assignment practice font and made the following additions:
...
![Bildschirmfoto 2025-05-02 um 22.24.35.png](https://res.craft.do/user/full/ce92c3bc-bf1f-2f0e-e611-560f02f61cc0/doc/37a2d212-9359-45ac-8a9b-ebdf231b405b/78d80379-2950-71a8-853e-b158abd15103/3NxoPLFuavZn2ifoDgmrrwuMdvUIyDnky0rN6FFJQAIz/Bildschirmfoto%202025-05-02%20um%2022.24.35.png)

*Two variants of the standard cut, each narrower by 1/4 stem width and two wider*
...
*Result with scaling to a maximum of 2 edges shows conspicuous differences in width in the first, fifth and seventh lines (9 pt Knuth-Plass, Unitized Scaling)*
...
*Result with scaling to a maximum of 1 edge shows hardly any differences in width, but still improved line formation due to a slight reduction in white space (9 pt Knuth-Plass, Unitized Scaling)*
...
*Result without scaling shows poorer line formation in the first and penultimate line due to the larger white space. (9 pt Knuth-Plass, no scaling)*
...
*The character set of the font used with additional variants of the lowercase letters, which are exactly one cadence wide.*
...
*The glyphs are positioned exactly on the cadence grid. On the left the normal section with a stem interval of 14 cadences per 18 1/1000 EM (based on 1/4 stem width), on the right the wider version with a stem interval of 16 cadences. [Axis wdth=120]*

- It is precisely this process that was already used by Gutenberg, and although it can be automated using modern technology, it cannot be improved. (Although the abbreviations and certain ligatures are no longer comprehensible to the common reader today).
...
*Example of a test text in italic font.*
...
The normal font is calibrated at 100, a wider variant at 120.
...
The normal variant is placed on the unitized grid. The wide variant is extended by 2 units in the counter, and one unit is also added to the left and right on the outside.
...
*Left the normal width n with 10 units in the counter, right the wide variant with 12 units in the counter.*
...
*Diagram for the text “In olden times ...” Penalty values (y-axis) for column widths from 192 to 576 pt (x-axis), red line = without scaling, blue line with unitized scaling, solid lines = with hyphenation, dashed = without hyphenation*
...
*Diagram for the text “In the beginning was ...” Penalty values (y-axis) for column widths from 192 to 576 pt (x-axis), red line = without scaling, blue line with unitized scaling, solid lines = with hyphenation, dashed = without hyphenation*
...
*Diagram for the text “By taking into ...” Penalty values (y-axis) for column widths from 192 to 576 pt (x-axis), red line = without scaling, blue line with unitized scaling, no values for hyphenation.*
...
*Example for the use of the function on websites*
...
![Image.tiff](https://res.craft.do/user/full/ce92c3bc-bf1f-2f0e-e611-560f02f61cc0/doc/37a2d212-9359-45ac-8a9b-ebdf231b405b/8c1b2cd4-a8dd-1bfb-ba07-a6193cba46a6/hiTSReRN0To6GhkA6v4E55QvPqxAimxsxGQCOlQLx8cz/Image.tiff)

*User interface of the Adobe Indesign PlugIn - work in progress*

---

# Jörg Drees

Born on the German North Sea coast in 1967, Jörg came into contact with hot metal typesetting through his parents' print shop. He graduated as a typographic designer in Zurich.

?descriptionFromFileType=function+toLocaleUpperCase()+{+[native+code]+}+File&mimeType=application/octet-stream&fileName=Just+with+Units.md&fileType=undefined&fileExtension=md
